...
//  Name:     resampler.h
//
//  Purpose:  Performs arbitrary rate resampling via a bank of sinc
//            interpolation filters
//
//  Created:  2012/09/24
//
//  Description:
//    Uses a bank of windowed sinc or SRRC interpolation filters to perform
//    arbitrary rate resampling.  The polyphase filter bank is created via
//    the sincbank.h class.  The phase accumulator for the resampler is
//    implemented as a double precision floating point number.  Although
//    resampling rate may be arbitrary, the phase precision (and thus phase
//    error) of a specific interpolated sample is determined by the number of
//    filters in the filter bank (more filters = smaller maximum phase error 
//    but filter bank takes up more memory).  Each interpolation phase is
//    rounded to the phase of the nearest filter in the bank to the desired
//    phase.
//
//    Filtering (interpolation) is performed in the time domain as a
//    dot-product between data and time-reversed filter.  An optimized
//    dot-product method is provided in evm.h if Streaming SIMD Extensions
//    (SSE) are supported on your architecture.  This faster math can be
//    enabled by compiling with -DHAVE_SSE, -DHAVE_SSE2, and -DHAVE_SSE3.
//    Input types can be real or complex float, output type will always be the
//    same as input type and the filter bank will always consist of real
//    (float) lowpass sinc interpolation filters.
//
//    Important note: to make full use of the optimizations in this clase,
//    you must either create a config.h file for your platform with the macro
//    definitions of the SSE/AVX optimizations supported and compile with
//    -DHAVE_CONFIG_H (see inc/config.h for examples) OR compile with
//    -DHAVE_SSE, -DHAVE_SSE2, and -DHAVE_SSE3.
//
//    The user can control number of taps per leg of the sinc interpolation
//    filter bank (implemented as a polyphase filter) as well as number of
//    filters in the bank.  Note that the phase precision that the interpolator
//    can achieve will be equal to 1/numFilters cycles.  Filter BW as a
//    fraction of the input sampling rate can also be controlled by the user
//    but note that there is no aliasing check so it is up to the user to
//    ensure that bw < output/input rate.
//    
//    Additionally, a windowing function can be applied to the sinc filter from
//    the list below:
//
